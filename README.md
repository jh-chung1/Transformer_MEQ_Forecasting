...
*Schematic of the forecasting procedure. $\bm{X}^{(k)}$ denotes the cumulative monitoring input and $\bm{Y}^{(k)}$ the corresponding forecast window; $k$ is the segment index. The forecasting range is $l_\text{future}$*
